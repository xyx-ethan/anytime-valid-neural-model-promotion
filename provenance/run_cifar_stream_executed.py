...
STRONG_LAMBDA_GRID = np.linspace(0.0, 1.0, 17, dtype=np.float64)[1:]
...
def subexponential_psi(
    values: np.ndarray,
    scale: float = EVIDENCE_SCALE,
) -> np.ndarray:
    """Return the sub-exponential CGF-like function of Choe and Ramdas."""
    if np.any(values < 0) or np.any(values >= 1.0 / scale):
        raise ValueError("Mixture parameters must lie in [0, 1 / scale)")
    scaled = scale * values
    return (-np.log1p(-scaled) - scaled) / scale**2


def promotion_summary(differences: np.ndarray) -> tuple[int | None, float]:
    """Summarize a finite-mixture e-process for the weak average null."""
...
def strong_null_promotion_summary(
    differences: np.ndarray,
) -> tuple[int | None, float]:
    """Summarize the product-mixture process for the stepwise strong null."""
    values = np.asarray(differences, dtype=np.float64)
    log_components = np.zeros(len(STRONG_LAMBDA_GRID), dtype=np.float64)
    threshold = math.log(1.0 / ALPHA)
    first_crossing: int | None = None
    final_log_evidence = -math.inf
    ...
    for index, difference in enumerate(values, start=1):
        with np.errstate(divide="ignore", invalid="ignore"):
            log_components += np.log1p(
                STRONG_LAMBDA_GRID * difference
            )
        ...
        if finite.any():
            ...
            ) - math.log(len(STRONG_LAMBDA_GRID))
        if first_crossing is None and final_log_evidence >= threshold:
            first_crossing = index
...
def main() -> None:
    ...
    summary = {
        "configuration": {
            ...
            "evidence_process": (
                "finite mixture of sub-exponential e-processes for the weak "
                "average null"
            ),
            "alpha": ALPHA,
